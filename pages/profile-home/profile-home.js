...
const { DEFAULT_FONT_SCALE, normalizeFontScale, getFontScaleLabel } = require('../../utils/font-scale');
const { getAppLoginStatus } = require('../../utils/app-login-status');
const { requestAlertSubscription } = require('../../utils/alert-subscription');
const { findProfileById, removeProfileFromStore } = require('../../utils/profile-store');
const {
...
const { calculateAge, formatPhoneWithSpaces } = require('../../utils/profile-detail');
...
function buildLatestRecordDisplay(record, profile) {
  ...
  return {
    ...
    statusText: status.detail ? `血压${status.label}${status.detail}` : `血压${status.label}`,
...
function buildEmergencyText(profile) {
  const emergencyName = String(profile && profile.emergencyContact && profile.emergencyContact.name || '').trim();
  const emergencyPhone = String(profile && profile.emergencyContact && profile.emergencyContact.phone || '').trim();
...
function buildMemberItems(members, currentUserId) {
  return (Array.isArray(members) ? members : [])
    .map((member) => {
      ...
      let displayName = user.nickname || '未命名成员';
      if (isSelf) {
        displayName = role === 'owner' ? '我（管理员）' : '我';
      }
      ...
      return {
        ...
        avatarFallback: buildInvitationNicknameInitial(user.nickname, isSelf ? '我' : '家'),
        displayName,
...
Page({
  ...
  syncProfileMeta() {
    ...
    const profile = currentProfileId ? findProfileById(currentProfileId) : null;
    ...
    const birthYearText = profile && profile.birthDate ? `${String(profile.birthDate).slice(0, 4)} 年出生` : '';

    this.setData({
      profiles,
      currentProfileId,
      hasProfile: Boolean(profile),
      profileTitle: profile && profile.name ? profile.name : '来自儿女的关心',
      profileName: profile && profile.name ? profile.name : '',
      profileInitial: buildInvitationNicknameInitial(profile && profile.name, '家'),
      ...
      canInviteCurrentProfile: profile ? canInvite(state, currentProfileId) : false,
      canManageCurrentProfile: profile ? canManage(state, currentProfileId) : false,
      canEditCurrentProfile: profile ? canEditProfile(state, currentProfileId) : false,
      relationshipRole: relationship ? relationship.role : '',
      activeRelationshipId: relationship ? relationship._id : '',
      activeRelationshipSubscribeAlerts: Boolean(relationship ? relationship.subscribeAlerts : true),
    });
  },
  ...
  async loadMembers(profileId) {
    ...
    if (cachedMembers && !store.isStale('members', profileId, MEMBER_STALE_THRESHOLD)) {
      return cachedMembers.slice();
    }

    const result = await memberService.listProfileMembers(profileId);
    const members = Array.isArray(result.members) ? result.members.slice() : [];
  ...
  async loadPageData(options = {}) {
    ...
    try {
      ...
      this.activeMedications = Array.isArray(medicationResult.activeMedications)
        ...
        : [];
      this.historicalMedications = Array.isArray(medicationResult.historicalMedications)
  ...
  applyViewModel(profile, medicationResult, members) {
    const activeMedications = Array.isArray(medicationResult.activeMedications)
      ? medicationResult.activeMedications
      : [];
    const historicalMedications = Array.isArray(medicationResult.historicalMedications)
      ? medicationResult.historicalMedications
      : [];
    const medicationText = buildMedicationSummary(activeMedications, 2);
    const medicationCount = activeMedications.length + historicalMedications.length;
    ...
    this.setData({
      ...
      errorText: '',
      latestRecordDisplay: buildLatestRecordDisplay(this.latestRecord, profile),
      hasLatestRecord: Boolean(this.latestRecord),
      hasMedicationSummary: Boolean(medicationText),
      medicationText,
      medicationCount,
      medicationShortcutText: medicationCount > 0 ? `已配置 ${medicationCount} 种用药` : '添加长期用药记录',
  ...
  handleOpenMedicationManagement() {
    ...
    const firstMedication = this.activeMedications[0] || this.historicalMedications[0];
    const url = firstMedication && firstMedication._id
  ...
  async handleToggleSubscribeAlerts(event) {
    ...
    if (subscribeAlerts && !previousValue) {
      await requestAlertSubscription(() => this.persistOwnSubscribeAlerts(subscribeAlerts, previousValue));
      return;
    }
  ...
  async handleDeleteProfile() {
    if (!this.data.currentProfileId || !this.data.canManageCurrentProfile || this.data.isDeletingProfile) {
      return;
    }
