...
function getDisplayPatient(patient, privacyMode) {
  ...
  return {
    nameText: privacyMode ? safePatient.maskedNameText : safePatient.rawNameText,
    medicationText: safePatient.medicationText || '暂无用药记录',
    emergencyText: privacyMode ? safePatient.maskedEmergencyText : safePatient.rawEmergencyText,
  };
}
...
function approximateLineCount(text, maxCharsPerLine) {
  ...
  return Math.max(1, Math.ceil(safeText.length / Math.max(1, maxCharsPerLine)));
}
...
function estimatePatientSectionHeight(patient) {
  ...
  const firstRowHeight = Math.max(62 + nameLines * 32, 62 + medicationLines * 32);
...
function measureReportExportHeight(payload) {
  ...
  if (height > MAX_EXPORT_HEIGHT && initialAlerts.items.length > 3) {
    ...
    reducedAlerts.hasMore = Array.isArray(payload.recentAlerts) && payload.recentAlerts.length > 3;
...
function drawReportExportCanvas(ctx, payload) {
  ...
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
  ctx.fillText('血压心率就诊报告', PADDING_X, y);
  ...
  const leftEndY = drawLabelValue(ctx, '患者姓名', patient.nameText, PADDING_X, y, columnWidth);
  const rightEndY = drawLabelValue(ctx, '主要用药', patient.medicationText, PADDING_X + columnWidth + columnGap, y, columnWidth);
  y = Math.max(leftEndY, rightEndY) + 18;
  y = drawLabelValue(ctx, '紧急联系人', patient.emergencyText, PADDING_X, y, CONTENT_WIDTH) + 6;

  if (payload.banner) {
    y += SECTION_GAP;
    const bannerBg = payload.banner.type === 'critical' ? '#FEE2E2' : '#FFEDD5';
    const bannerTitle = payload.banner.type === 'critical' ? '#991B1B' : '#9A3412';
    const bannerText = payload.banner.type === 'critical' ? '#7F1D1D' : '#7C2D12';
    const bannerHeight = estimateBannerHeight(payload.banner);

    drawRoundedRect(ctx, PADDING_X, y, CONTENT_WIDTH, bannerHeight, 22, bannerBg);
    ...
    drawWrappedText(ctx, payload.banner.text, PADDING_X + 24, y + 58, CONTENT_WIDTH - 48, 24);
    y += bannerHeight;
  }
  ...
  if (payload.hasRecords) {
    ...
    (payload.summaryCards || []).forEach((item, index) => {
      ...
      ctx.fillStyle = item.accentClassName === 'report-summary__value--danger'
        ? '#B42318'
    ...
    if (payload.hasHeartRateData) {
      ...
      drawRoundedRect(ctx, PADDING_X, y, CONTENT_WIDTH, 92, 18, '#F8FAFC');
      ctx.fillStyle = '#6B7280';
      ctx.font = '15px sans-serif';
    ...
    if (exportAlerts.length) {
      exportAlerts.forEach((item, index) => {
        ...
        ctx.font = '14px sans-serif';
        const alertEndY = drawWrappedText(ctx, item.alertText, PADDING_X, y + 24, 360, 22);
